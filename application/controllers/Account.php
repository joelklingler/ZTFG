        ...
        if(!isset($user))
        {
            ...
            if(!password_verify($pw, $user->passwordhash)) {
                ...
                if(!$this->db->query($sql_query))
                {
                    ...
                    // Load role
                    $role = $this->db->query("SELECT * FROM roles WHERE id=".$user->roleId.";")->row();
                    if(!isset($role))
                    {
                        $operationMessage = "Interner Fehler";
                    }
                    
                    $actions = array();
                    // Get actions
                    $actions = $this->db->query("SELECT * FROM actions WHERE minRoleLevel <= ".$role->level." ORDER BY minrolelevel ASC;")->result_array();                  
